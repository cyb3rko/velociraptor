...
class CacheLimitProvider(private val context: Context) : LimitProvider {
    ...
    fun get(previousRoadName: String?, coord: Coord): List<LimitResponse> {
        ...
        val selectedWays = db.wayDao().selectByCoord(
            coord.lat,
            cos(Math.toRadians(coord.lat)).pow(2.0),
            coord.lon
        )

        val onPathWays = selectedWays.filter { (_, _, _, _, lat1, lon1, lat2, lon2) ->
    ...
    private fun getRoadNameSimilarity(way: Way, previousRoadName: String?): Double {
        if (previousRoadName == null)
            ...
            val roadLower = way.road.lowercase()
            val prevRoadNameLower = previousRoadName.lowercase()
            ...
            return if (roadNameSplittable && !previousRoadNameSplittable) {
                val split = roadLower.split(OsmLimitProvider.ROADNAME_DELIM)
                normalleven.similarity(split[0], prevRoadNameLower)
                    .coerceAtLeast(normalleven.similarity(split[1], prevRoadNameLower))
                ...
                normalleven.similarity(roadLower, split[0])
                    .coerceAtLeast(normalleven.similarity(roadLower, split[1]))
            } else {
                normalleven.similarity(roadLower, prevRoadNameLower)
            }
        }
    }
    ...
    companion object {
        ...
        private fun isLocationOnPath(p1: Coord, p2: Coord, t: Coord): Boolean {
            val latLngs = listOf(p1.toLatLng(), p2.toLatLng())
